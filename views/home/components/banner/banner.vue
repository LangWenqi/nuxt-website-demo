			...
			<div v-swiper:bannerSwiper="swiperOption" v-if="bannerList && bannerList.length > 0">
				<div class="swiper-wrapper">
					<div class="swiper-slide" v-for="(bannerItem) in bannerList" :key="bannerItem.id">
						<div class="home__banner__img" @click="navigateTo(bannerItem)">
							<img class="img" :src="bannerItem.image" alt="">
						</div>
					</div>
				</div>
			</div>
			<div class="home__icon home__icon__left cursor" @click="prev"></div>
			<div class="home__icon home__icon__right cursor" @click="next"></div>
		</div>
</template>
